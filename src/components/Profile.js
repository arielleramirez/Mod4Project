...
class Profile extends Component {
  ...
  handleDelete = recipe => {
    ...
    if (recipe.user_id === this.props.currentUser.id) {
      ...
      this.setState({
        userRecipes: [
          ...this.state.userRecipes.splice(0, position),
          ...this.state.userRecipes.splice(position + 1)
        ]
      });
    } else {
      ...
      this.setState({
        userCollections: [
          ...this.state.userCollections.splice(0, position),
          ...this.state.userCollections.splice(position + 1)
        ]
      });
    }
